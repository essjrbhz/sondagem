...
export default function PanoramaOperacional() {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-primary">Panorama Operacional</h1>
          <p className="text-sm text-gray-400 mt-1">Atualizado em {syncTime}</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium bg-primary-50 text-primary">
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {loadingKPIs ? (
          ...
          <>
            <KPICard numero={kpis?.campanhas_ativas ?? '—'}                          label="Campanhas Ativas"  cor="primary" />
            <KPICard numero={kpis?.rdos_total.toLocaleString('pt-BR') ?? '—'}        label="RDOs Totais"       cor="accent"  />
            <KPICard numero={kpis ? fmtMetros(kpis.metros_executados_total) : '—'}   label="Metros Executados" cor="gold"    />
            <KPICard numero={kpis?.obras_ativas ?? '—'}                              label="Obras Ativas"      cor="primary" />
          </>
        )}
      </section>
      ...
      <section>
        ...
        {loadingAlertas ? (
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm bg-green-50 border border-green-200 text-green-700">
      ...
      <section>
        ...
        {loadingCampanhas ? (
          ...
          <div className="bg-white rounded-lg shadow-card p-8 text-center text-gray-400 text-sm">
